...
The live example composes **Button as the trigger**: a Button animates through `hover` → `press` states (the "click"), the select panel opens right after the press lands, and rows animate through hover → press → selected as the user picks an option. Copy the code to drop it into your own composition.

<LiveExample name="select-example" />
